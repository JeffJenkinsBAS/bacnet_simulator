...
async def test_validator_rejects_safety_bypass_on_a_non_safety_point():
    registry = PointRegistry([_vav_group()])
    registry.build_objects()
    validator = ActionValidator(registry)
    ...
    bundle = LlmActionBundle(
        request_id="r1",
        intent=AllowedIntent.inject_fault,
        summary="invalid safety bypass target",
        actions=[
            LlmAction(
                type=AllowedActionType.inject_fault,
                group_id="ACI-SIM-VAV-1",
                alias="airflow",
                fault_type="safety_bypass",
            )
        ],
    )
    result = validator.validate_bundle(bundle)
    ...
    assert result.valid is False
    assert any("safety_bypass may target only" in error for error in result.errors)
